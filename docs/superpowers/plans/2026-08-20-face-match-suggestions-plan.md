...
- [ ] This plan does not cover production deployment (running the schema against every production school DB, setting production env vars/secrets, confirming the production `uploads/{school}/childphotos/` path Task 8 reads from is actually reachable at `dirname(base_path(),1)` on the production server) — treat that as a separate, deliberate rollout step once local verification is complete, not an automatic follow-on.
